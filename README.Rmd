...
If you set the parameter `as_perc` to `FALSE`, the CV will not be multiplied by 100.
...
### format_digit
The goal of *format_digit* is to add zero on left of a number, so that all values have the same number of characters.
...
The goal of *tbl_chi_square* is to create a frequency table with chi-square statistic, p-value, Cramer's V.
...
The goal of *is_outlier* is to check if a value is an outlier, by the boxplor criteria.
...
The goal of *parse_text* is to simulate data, where two variables will be linear correlated using pearson correlation coefficient.
